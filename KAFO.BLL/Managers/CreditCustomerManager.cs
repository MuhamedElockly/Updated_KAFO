	...
	public class CreditCustomerManager
	{
		...
		public CreditCustomerAccountData GetWithInvoicesData(int customerId)
		{
			...
			var customer = _unitOfWork.CustomerAccounts.Get(
				c => c.Id == customerId,
				includeProperties: "Deposits,Deposits.User,Withdraws"
			);
			if (customer == null) return null;

			var deposits = customer.Deposits != null
				? customer.Deposits.OrderByDescending(i => i.CreatedAt)
					.Select(i => new CreditCustomerTransactionData
					{
						...
						SellerName = i.User?.Name ?? "غير معروف",
						Time = i.CreatedAt,
						Amount = i.TotalInvoice
					}).ToList()
				: new List<CreditCustomerTransactionData>();
			var withdraws = customer.Withdraws != null
				? customer.Withdraws.OrderByDescending(i => i.CreatedAt)
					.Select(i => new CreditCustomerTransactionData
					{
						IsDeposit = false,
						SellerName = i.User?.Name ?? "غير معروف",
			...
			var allTransactions = new List<CreditCustomerTransactionData>();
			allTransactions.AddRange(deposits);
			allTransactions.AddRange(withdraws);
			allTransactions = allTransactions.OrderByDescending(i => i.Time).ToList();
